      ...
      {isOpen && (
        <div className="cart-body">
          {cartItems.length === 0 ? (
            ...
            >
              <img
                src="./images/icons/icon-cart-default.svg"
                alt="empty"
              ...
              />
              <p className="mb-0">　購物車沒有東西呀！快挑選產品吧:）</p>
            </div>
          ) : (
            <>
              ...
              {cartItems.map((item) => (
                <div className="row align-items-center border-bottom py-3" key={item.id}>
                  ...
                  <div className="col-2">
                    ...
                    <div><Tag label="檔期活動 10% off" color="sale" /></div>
                  </div>

                  <div className="col-1">NT${item.price}</div>

                  <div className="col-2 d-flex gap-2 align-items-center">
                    <button className="quantity-btn" onClick={() => handleQtyChange(item.id, -1)}>−</button>
